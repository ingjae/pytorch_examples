...
print(train_dataset.get_countries())
...
def wrap_model(model):
    ...
    if torch.cuda.is_available():
        ...
        pass
# pad sequences and sort the tensor # zero padding 
def pad_sequences(vectorized_seqs, seq_lengths, countries):
    seq_tensor = torch.zeros((len(vectorized_seqs), seq_lengths.max())).long() # 가장 길게 제로 텐서 생성 
...
def make_variables(names, countries): # 아스키 코드로 변경 
...
class RNNClassifier(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, n_layers=1, bidirectional=True):
        ...
        self.embedding = nn.Embedding(input_size, hidden_size) # input size 가 왜 128
    ...
    def forward(self, input, seq_lengths):
        ...
        # Pack them up nicely
        gru_input = pack_padded_sequence( # 패키징 
            embedded, seq_lengths.data.cpu().numpy()) #data , batch size 
